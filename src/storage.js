...
const loadLargeSyncState = () => new Promise((resolve, reject) => {
...
export const loadState = async (callback) => {
    ...
    try {
        ...
        if (oldData) {
            console.debug("loading largeSync data format, skipping check for new data in plain format", oldData);
            callback(oldData);
            return;
        }
        chrome.storage.sync.get(null, (data) => {
            ...
            if (!data || Object.keys(data).length === 0) {
                console.debug("initial data from sync is empty, trying local storage", data);
